...
EOF
  printf '%b' "$NC"
  printf '  %bACS — Air Crack Station%b\n' "${GREEN}${BOLD}" "$NC"
  printf '  %bMade by %s%b\n' "${YELLOW}${BOLD}" "$ACS_AUTHOR" "$NC"
  printf '  %bhttps://github.com/iinze0/ACS%b\n' "${CYAN}" "$NC"
  printf '  %bv%s%b\n\n' "${GREEN}${BOLD}" "$ACS_VER" "$NC"
}
...
ACS_VERSION_URL="https://raw.githubusercontent.com/iinze0/ACS/main/VERSION"
...
fetch_remote_ver() {
  local remote=""
  remote=$(curl -fsSL --max-time 12 -A "acs-update" "$ACS_VERSION_URL" 2>/dev/null | tr -d '[:space:]') || remote=""
  if [[ -z $remote ]]; then
    remote=$(curl -fsSL --max-time 12 -A "acs-update" "$ACS_RAW" 2>/dev/null | sed -n 's/^ACS_VER="\([^"]*\)"/\1/p' | head -n1) || remote=""
  fi
  printf '%s' "$remote"
}

apply_update() {
  local remote tmp dest url
  ...
  echo -e "${CYAN}Downloading ACS v${remote}…${NC}"
  tmp="/tmp/acs_${remote}_all.deb"
  url="https://github.com/iinze0/ACS/releases/download/v${remote}/acs_${remote}_all.deb"
  if curl -fL --retry 3 --max-time 60 -A "acs-update" -o "$tmp" "$url" && dpkg-deb -I "$tmp" >/dev/null 2>&1; then
    dpkg -i "$tmp"
...
auto_update() {
  ...
  have curl || return 0
  local remote
  remote=$(fetch_remote_ver) || return 0
...
[[ $EUID -ne 0 ]] && { bad "run: sudo ACS"; exit 1; }
mkdir -p "$SESSION_DIR"
auto_update
show_banner
